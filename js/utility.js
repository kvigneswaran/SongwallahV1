...
var Utility = {
   ...
   debug: function(obj) {
	   var dt = this.formatDateTime(new Date()) ;
	    var temp = dt + ":" + obj;
		console.debug(temp);
		syslog += '<span>' + dt + '</span>' + ':' + obj + '<br>';
	},	
	getSysLog : function() {
		return syslog;
	},
	loadCoverArts: function($http, clips, startFrom) {
		var allclipids = "";
		Utility.debug("Loading cover arts from index:" + startFrom);
		for (var j = startFrom; j < clips.length; j++) {
			if(clips[j].clipData !== undefined)
				allclipids += clips[j].clipData.ID + "~";
			else
		...
		if (allclipids != "") {
	         ...
	         //   $scope.status = status;
				...
				//	debug(data);
					...
					angular.forEach(coverarts.processedResults,function(coverart, key) {
					    if (coverart != null) {
                            var coverartimagebytes = "";
                            if (coverart.coverArtImageBytes !== undefined)
                                    coverartimagebytes = coverart.coverArtImageBytes;
                            else
                                    coverartimagebytes = coverart;

                            if (coverartimagebytes != "") {
                                    coverartimagebytes = LZString.decompressFromBase64(coverartimagebytes);
                                    if(clips[key].clipData !== undefined)
                                            clips[key].clipData.coverArtImageBytes = coverartimagebytes;
                                    else
	        ...
	        .error(function(data, status) {
	            ...
	            //$scope.status = status;
	        	Utility.debug("Request failed");
	        });
	    }				
	},
	 enhanceDataForServiceCall : function (data) {
		data["songwallah.user"] = window.localStorage.getItem("songwallah.user");
		data["songwallah.guest"] = Utility.getGuestId();
		data["songwallah.user.name"] = window.localStorage.getItem("songwallah.user.name");
		data["songwallah.user.profile.pic.url"] = window.localStorage.getItem("songwallah.user.profile.pic.url");
		data["songwallah.user.type"] = window.localStorage.getItem("songwallah.user.type");
		data["songwallah.user.fb.uid"] = window.localStorage.getItem("songwallah.user.fb.uid");
		data["songwallah.user.fb.longtermtoken"] = window.localStorage.getItem("songwallah.user.fb.longtermtoken");
		Utility.debug("songwallah.guest:" + data["songwallah.guest"]);
		return data;
	},
	getUserId : function() {
		var userId =window.localStorage.getItem("songwallah.user"); 
	...
	getGuestId : function() {
		var guestId = window.localStorage.getItem("songwallah.guest");
		if (guestId == null) {
			guestId = Utility.getRandomString(32, '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ');
			window.localStorage.setItem("songwallah.guest", guestId);
		}
		return guestId;
	},
	...
	updateClipStats: function(clip, $http) {
		var userId = window.localStorage.getItem("songwallah.user");
		var timeStamp = new Date().getTime();
		if (userId == null) {
			userId = Utility.getGuestId();
		}
		...
		$http({
			...
			data : {
				...
				c: clip.ID,
				t: timeStamp,
				"songwallah.user" : userId
			},
			headers: {'Content-Type' : 'application/json'}
		});
		
		var recentClips = [{clipData: clip, ts: timeStamp}];
		if (window.localStorage.getItem("recentClips") != null) {
			recentClips = recentClips.concat(JSON.parse(window.localStorage.getItem("recentClips"))).slice(0,5)
		}
		
		window.localStorage.setItem("recentclips", JSON.stringify(recentClips));
	},
	formatDateTime : function(dt) {
		if (dt != null && dt instanceof Date) {
			return dt.getUTCFullYear() + "-" + (dt.getUTCMonth() +1)+ "-" + dt.getUTCDate() + "T" + dt.getUTCHours() + ":" + dt.getUTCMinutes() + ":"+dt.getUTCSeconds()+"Z";
		}
		else {
			return "";
		}
